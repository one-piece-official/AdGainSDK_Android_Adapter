...
public class ExpandAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = LayoutInflater.from(mContext).inflate(R.layout.tobid_callback_item, parent, false);
        ...
        if (callItem.is_callback()) {
            viewHolder.callBackText.setTextColor(mContext.getResources().getColor(R.color.tobid_color_accent));
        } else {
            viewHolder.callBackText.setTextColor(mContext.getResources().getColor(R.color.tobid_color_gray));
        }
        ...
        if (TextUtils.isEmpty(callItem.getChild_text())) {
            ...
            if (callItem.is_expand()) {
                ...
                viewHolder.expandImage.setImageDrawable(ContextCompat.getDrawable(mContext, R.mipmap.tobid_image_arrow_down));
            } else {
                viewHolder.infoText.setVisibility(View.GONE);
                viewHolder.expandImage.setImageDrawable(ContextCompat.getDrawable(mContext, R.mipmap.tobid_image_arrow_right));
            }
        }
